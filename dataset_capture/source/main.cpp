...
pcl::PointCloud<pcl::PointXYZRGBNormal>::Ptr new_model_3d(new pcl::PointCloud<pcl::PointXYZRGBNormal>); 
...
public:
  ...
  drawResult (pcl::visualization::PCLVisualizer& viz)
  {
    ...
    pcl::PointCloud<pcl::PointXYZRGBNormal>::Ptr new_model_transformed_cloud (new pcl::PointCloud<pcl::PointXYZRGBNormal> ());
    ...
    if(!template_alignment)
    {
      ...
      //pcl::PointCloud<pcl::PointXYZRGBNormal>::Ptr environment_cloud (new pcl::PointCloud<pcl::PointXYZRGBNormal> ());
      //pcl::copyPointCloud(*cloud_pass_downsampled_, *environment_cloud);
      //pcl::io::savePLYFileASCII ("background.ply",*environment_cloud);
    ...
    viewer->addPointCloud<pcl::PointXYZRGBNormal> (transformed_cloud, "transformed_cloud");
    viewer->addPointCloud (cloud_pass_, "cloudpass");
  ...
  cloud_cb (const CloudConstPtr &cloud)
  {
    ...
    pcl::PointCloud<pcl::PointXYZRGBA>::Ptr pc_xyzrgba(new pcl::PointCloud<pcl::PointXYZRGBA>);  
  
    int number_frames_to_load = 1;
    cv::Mat image;
    cout<<"counter_: "<<counter_<<endl;
    loadDataset(dataset_path, *pc_xyzrgba, counter_, number_frames_to_load, rgb_image);
    ...
    filterPassThrough (pc_xyzrgba, *cloud_pass_);
    
    int warmup_frames = 0;

    if (counter_ < warmup_frames)
    {
      gridSample (cloud_pass_, *cloud_pass_downsampled_, downsampling_grid_size_);
    }
    else if (counter_ == warmup_frames)
    {
...
TextureMesh loadMesh(std::string path)
{
...
usage (char** argv)
{
  std::cout << "usage: " << argv[0] << " <device_id> -dataset <path> -model3d <path> [-C] [-g]\n\n";
  ...
  std::cout << "  -d <value>: specify the grid size of downsampling (defaults to 0.01)."
            << std::endl;
  std::cout << "  -dataset <path>: specify the dataset folder"
            << std::endl;
  std::cout << "  -model3d <path>: specify the model3d location and name (without the .ply extension!)."
            << std::endl;
}
...
main (int argc, char** argv)
{
  ...
  pcl::console::parse_argument (argc, argv, "-dataset", dataset_path);
  pcl::console::parse_argument (argc, argv, "-model3d", model3d_path);
  if (argc < 2)
  {
    usage (argv);
    exit (1);
  }
  pcl::io::loadPLYFile(model3d_path+("_with_normals.ply"), *model_3d); 
  mesh = loadMesh(model3d_path+".ply");
